...
Verification Portal: https://verify.eventpulse.org`,
  ...
  templateConfig: {
    type: 'preset', // 'preset' | 'upload' | 'canva'
    presetId: 'tech_summit',
    customImage: null,
    canvaUrl: ''
  },
...
function loadFullDemoProject() {
  loadDemoRoster();
  selectPresetTemplate('tech_summit');
  navigateToStep(2);
  showToast("Interactive demo project loaded!", "success");
}

// ==========================================================================
// 2. Step 2: Certificate Template & Visual Canvas Mapper
// ==========================================================================
function switchTemplateSource(source) {
  state.templateConfig.type = source;

  document.querySelectorAll('.tab-btn').forEach(btn => btn.classList.remove('active'));
  document.querySelectorAll('.source-subpanel').forEach(p => p.classList.remove('active'));

  if (source === 'preset') {
    document.getElementById('tabPreset').classList.add('active');
    document.getElementById('sourcePreset').classList.add('active');
  } else if (source === 'upload') {
    document.getElementById('tabUpload').classList.add('active');
    document.getElementById('sourceUpload').classList.add('active');
  } else if (source === 'canva') {
    document.getElementById('tabCanva').classList.add('active');
    document.getElementById('sourceCanva').classList.add('active');
  }
...
function selectPresetTemplate(presetId) {
  state.templateConfig.type = 'preset';
  state.templateConfig.presetId = presetId;
...
function applyCanvaDesignLink() {
  const url = document.getElementById('canvaLinkInput').value.trim();
  if (!url) {
    showToast("Please enter a Canva design URL", "error");
    return;
  }
  state.templateConfig.type = 'canva';
  state.templateConfig.canvaUrl = url;
  showToast("Connected Canva link! Using aligned layout canvas.", "success");
  testRenderSampleCertificate();
}
...
function renderCertificate(canvas, attendeeData = null, isEditing = false) {
  ...
  if (state.templateConfig.type === 'upload' && state.templateConfig.customImage) {
    ctx.drawImage(state.templateConfig.customImage, 0, 0, w, h);
  } else {
    drawPresetBackground(ctx, w, h, state.templateConfig.presetId);
  }
...
function drawPresetBackground(ctx, w, h, presetId) {
  if (presetId === 'royal_gold') {
    // Royal Gold Academic Theme
    ...
    ctx.fillText("HONORARY ACADEMIC COUNCIL & SUMMIT BOARD", w / 2, 130);

    ctx.font = "900 42px 'Cinzel', serif";
    ctx.fillStyle = "#ffffff";
    ctx.fillText("CERTIFICATE OF EXCELLENCE", w / 2, 200);

    ctx.font = "400 18px 'Playfair Display', serif";
    ctx.fillStyle = "#d6d3d1";
    ctx.fillText("This is officially and proudly conferred upon", w / 2, 280);

    ctx.font = "400 17px 'Playfair Display', serif";
    ctx.fillStyle = "#a8a29e";
    ctx.fillText("for distinguished active attendance and scholarly participation in", w / 2, 500);

    // Signatures
    ctx.strokeStyle = "#78716c";
    ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(180, 680); ctx.lineTo(400, 680); ctx.stroke();
    ctx.font = "600 14px 'Plus Jakarta Sans', sans-serif";
    ctx.fillStyle = "#a8a29e";
    ctx.fillText("DATE OF ISSUE", 290, 705);

    ctx.beginPath(); ctx.moveTo(800, 680); ctx.lineTo(1020, 680); ctx.stroke();
    ctx.fillText("AUTHORIZED DEAN", 910, 705);
    ctx.fillStyle = "#ffffff";
    ctx.fillText("Dr. S. K. Mukherjee", 910, 730);

  } else if (presetId === 'cyber_minimal') {
    // Cyber Emerald Modern Theme
    const bgGrad = ctx.createLinearGradient(0, 0, w, h);
    ...
    ctx.textAlign = "center";
    ctx.fillText("GLOBAL HACKATHON & DEVELOPER SYMPOSIUM", w / 2, 140);
    ...
    ctx.fillText("RECOGNITION OF DEDICATION & ATTENDANCE", w / 2, 290);

    ctx.font = "400 17px 'Plus Jakarta Sans', sans-serif";
    ctx.fillStyle = "#6ee7b7";
    ctx.fillText("has fulfilled all workshop and event requirements for", w / 2, 500);
    ...
    ctx.beginPath(); ctx.moveTo(800, 680); ctx.lineTo(1020, 680); ctx.stroke();
    ctx.fillText("EVENT CHAIRPERSON", 910, 705);
    ctx.fillStyle = "#ffffff";
    ctx.fillText("Prof. Elena Rostova", 910, 730);

  } else {
    // Default: Tech Summit Modern Indigo / Rose
    const bgGrad = ctx.createLinearGradient(0, 0, w, h);
    ...
    ctx.textAlign = "center";
    ctx.fillText("OFFICIAL EVENT VERIFICATION", w / 2, 140);
    ...
    ctx.fillText("PROUDLY AND FORMALLY PRESENTED TO", w / 2, 290);

    // Underline beneath name spot
    ...
    ctx.fillStyle = "#cbd5e1";
    ctx.fillText("for exemplary and active attendance in the technical conference", w / 2, 505);
    ...
    ctx.fillStyle = "#94a3b8";
    ctx.fillText("DATE OF ISSUANCE", 290, 705);
    ...
    ctx.fillText("AUTHORIZED SIGNATURE", 910, 705);
    ctx.fillStyle = "#ffffff";
    ctx.fillText("Convener & Chair", 910, 730);

    // Central Gold Seal
    ctx.strokeStyle = "#fbbf24";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.arc(w / 2, 675, 40, 0, Math.PI * 2);
    ctx.stroke();
...
function loadTemplateImageFile(file) {
  ...
  reader.onload = (e) => {
    ...
    img.onload = () => {
      ...
      testRenderSampleCertificate();
      showToast("Template image loaded successfully!", "success");
    };
